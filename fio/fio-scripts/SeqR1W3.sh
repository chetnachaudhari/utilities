...
do
	mkdir /grid/$grid/fio-$testName
		...
		touch $fileName
                start_time=`date +%s`   
		fio --filename=$fileName --ioengine=sync --direct=1 --gtod_reduce=1 --iodepth=64 --readwrite=readwrite --rwmixread=25 --name SeqR1W3 --sync=1 --bs=$bsize  --numjobs=`nproc` --time_based --runtime=30 --group_reporting --norandommap --size=4G
                end_time=`date +%s`;
