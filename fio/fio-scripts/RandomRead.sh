...
do
	mkdir /grid/$grid/fio-$testName
		...
		touch $fileName
                start_time=`date +%s`   
		fio --filename=$fileName --randrepeat=1 --ioengine=sync --direct=1 --gtod_reduce=1 --iodepth=64 --readwrite=randread --name RandomRead --sync=1 --bs=$bsize  --numjobs=`nproc` --time_based --runtime=30 --group_reporting --norandommap --size=4G
                end_time=`date +%s`;
